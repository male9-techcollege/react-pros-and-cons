...
Screenshot of a typescript type extending the InputHTMLAttributes interface.
The full code is available at the sandbox link below."
https://frontguys.fr/front-end/typescript-react-native-attributes/

Course notes/reminder: Kasper wrote on Teams (about putting a function in an interface):
...
Reminder: a void function is a function at the end of its call stack if I understood that concept properly.
The following function (a void function) does not receive arguments, and it does not return anything.
onClick?: ()=>void;
The following function, from Kasper's example, is a void function that takes an argument of type string.
(myArgument: string) => void
The following function, also from Kasper's example, returns something, so it's not void.
() => myString: string

Source for the following key-value pair (for an interface):
